...
def update_video_title(video_id: str):
    """Update the title of a video in the database."""
...
def get_video_last_index(video_id: str):
    ...
    last_index = service.get_video_last_index(db.session, video_id)
...
def update_video_last_index(video_id: str):
    ...
    service.update_video_last_index(db.session, video_id, last_index)
    return jsonify('success'), 200