...
def add_new_words(session: Session, words_data: list[dict[str, int | str]]):
    ...
    words = [LearningWord(**data) for data in words_data]
    new_words = get_new_words(session, words)
    words_repository.add_user_words(session, new_words)
    session.commit()
    return new_words


def get_new_words(session: Session, words: list[LearningWord]):
...
def get_pinyin(session: Session, word_id: int):
    ...
    word = words_repository.get_user_word_by_id(session, word_id)
    return word.pinyin
...
def get_proficiency_levels(session: Session, word_ids: list[int]):
    proficiency_levels = {}
    for id in word_ids:
        proficiency_levels[id] = words_repository.get_user_word_by_id(session, id).proficiency
...
def calculate_new_proficiency_levels(session: Session, previous_word_ids: list[int], current_word_id: int):
    ...
    previous_proficiency_levels = {id: words_repository.get_user_word_by_id(session, id).proficiency for id in previous_word_ids}
    ...
    current_proficiency_level = words_repository.get_user_word_by_id(session, current_word_id).proficiency
...
def calculate_hsk_percentages(session: Session):
    ...
    words: list[str] = [word.text for word in words_repository.get_all_words_for_user(session)]
...
def get_low_words(session: Session, threshold: int = 5):
    saved_words = words_repository.get_saved_words_for_user(session)
