...
import { state, wordsArea } from "../document-areas.js";
...
export async function printTranscript(transcript, newWords = false) {
    ...
    for (const line of transcript) {
        ...
        wordsArea.appendChild(transcriptLineElement);

        const text = line.text;
        wordIndex = await printText(text, wordIndex, false, newWords, transcriptLineElement);
 ...
 */
export async function printText(text, wordIndex = 0, clearArea = true, addNewWords = true, parentElement = wordsArea) {
    if (clearArea) {
